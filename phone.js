(()=>{
 ...
 let state=null,busy=false,target=null;
 async function refresh(){try{
  const r=await fetch('/api/phone-update',{cache:'no-store'});if(!r.ok)throw Error();state=await r.json();
  ...
  button.hidden=false;button.disabled=!state.canUpdate||busy||state.requestRunning;button.textContent=state.canUpdate?'Update trader':state.requestRunning||state.state==='APPLYING'?'Updating…':'Update trader · up to date';
 }catch{title.textContent='Update status unavailable';button.disabled=true;}}
