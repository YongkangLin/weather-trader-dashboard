(()=>{
 ...
 button.textContent='Update';box.append(title,note,button);
 ...
 async function refresh(){
  ...
  title.textContent=state.requestRunning?'Update requested':state.state==='AVAILABLE'?'A new release is ready':state.state==='APPLYING'?'Update is applying':state.state==='CURRENT'?'Up to date':'Trader updates';
  note.textContent=(state.releaseId||'')+' · '+(state.summary||state.state||'Waiting for release status');note.hidden=state.state==='CURRENT';button.title=note.textContent;
  ...
  button.textContent=state.canUpdate?'Update':state.requestRunning||state.state==='APPLYING'?'Updating…':'Update';
