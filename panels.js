...
function renderProductDesk(data){
 ...
 el.innerHTML=['weather','btc'].map(id=>{
  ...
  return `<article class="product-card" data-product="${id}"><div class="product-card-head"><div><span class="product-symbol">${isBTC?'₿':'☀'}</span><h3>${title}</h3></div><span class="product-pill ${enabled&&applied&&live?'on':''}">${escapeHTML(state)}</span></div><p class="product-scope">${escapeHTML(windowText)}</p>${metrics}<p class="product-pnl-note">${escapeHTML(pnlNote)}</p>${riskText}<p class="product-reason">${escapeHTML(reason)}</p><button type="button" class="product-toggle ${enabled?'pause':'enable'}" data-product="${id}" ${!supported||productBusy?'disabled':''}>${enabled?'Pause '+title+' entries':'Enable '+title+' entries'}</button><details data-product="${id}" ${opened.has(id)?'open':''}><summary>${isBTC?'Price chart · ':''}${a?.orderCount??'—'} orders · ${a?.positions?.length??'—'} open positions${partial?' · '+a.pendingMarkets+' P&L pending':''}</summary>${isBTC?btcPriceChart(data.btcChart):''}<h4>Positions</h4><ul>${positions||'<li>No recorded open positions</li>'}</ul><h4>Recent orders</h4><ul>${orders||'<li>No recorded orders</li>'}</ul><p>${a?.accountingStatus==='INCOMPLETE_HISTORY'?'Some historical fills lack complete evidence. Only known amounts are shown.':'P&L uses owned fills, actual fees and matched settlement receipts.'}</p></details></article>`;
 }).join('');
...
function btcPriceChart(chart){
 if(!chart?.points?.length)return `<section class="btc-chart"><h4>Bitcoin price movement</h4><p>${chart?.status==='UNAVAILABLE'?'Public BRTI prices are temporarily unavailable.':'Loading public BRTI prices…'}</p></section>`;
