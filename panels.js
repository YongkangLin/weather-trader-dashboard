...
function renderSharedReserve(data){
 const el=document.getElementById('shared-reserve-breakdown'),r=data.productRisk,c=data.capital;
 if(!el)return;
...
function renderProductDesk(data){
 ...
 el.innerHTML=['weather','btc'].map(id=>{
  ...
  const cooldownNs=Number(risk?.cooldownUntilNs||0),cooldownMinutes=modeLabel==='cooldown'&&cooldownNs>Date.now()*1e6?Math.max(1,Math.ceil((cooldownNs/1e6-Date.now())/60000)):0;
  const riskText=risk?`<dl class="product-metrics">${stat('Base share · '+(Number(risk.allocationFraction)*100).toFixed(0)+'% after reserve',risk.allocationUSD)}${stat('Current risk budget · '+(Number(risk.riskAllocationFraction??1)*100).toFixed(0)+'%',risk.riskAllocationUSD??risk.allocationUSD)}${stat('Own total exposure cap',risk.limits?.totalUSD)}${stat('Available cash capacity',risk.cashCapacityUSD)}</dl><p class="product-risk">Brake: ${escapeHTML(modeLabel)}${cooldownMinutes?' · '+cooldownMinutes+' min remaining':''}${modeLabel==='recovery'?' · 25% risk budget for 24h, then normal budget resumes if stable.':''} · active daily ${(Number(risk.activeDailyDrawdownFraction??risk.dailyDrawdownFraction??0)*100).toFixed(1)}% / 5% · active peak ${(Number(risk.activePeakDrawdownFraction??risk.peakDrawdownFraction??0)*100).toFixed(1)}% / 10%${risk.reason?' · '+escapeHTML(risk.reason.replaceAll('_',' ').toLowerCase()):''}</p>`:'<p class="product-risk">Your saved share after reserve · separate exposure limits and recoverable drawdown brake. Awaiting updated runtime.</p>';
...
function btcPriceChart(chart){
 if(!chart?.points?.length)return `<section class="btc-chart"><h2>Bitcoin · 15-minute price movement</h2><p>${chart?.status==='UNAVAILABLE'?'Public BRTI prices are temporarily unavailable.':'Loading public BRTI prices…'}</p></section>`;
